...
%< @version 1.2.0 Make ramp timing configurable
        ...
        function    ErrCod      =   RfMeas(obj, varargin)   
            ...
            if nargin > 2
                ...
                if stMod == 'Adi'
                    ...
                    fsSel                   =   [1, 1.2, 1.8].*1e6;
                    if isfield(Cfg, 'fs')
                        [Val ClkSel]        =   min(abs(fsSel - Cfg.fs));
					...
					if obj.Rf_TRampUp/ClkInt < 260
                        warning('Adf24Tx2Rx4: Set TRampUp to 260 clock cycles')
                        Cfg.TRampUp     =   260*ClkInt;                        
                    end
                    
                    
                    if obj.Rf_TRampDo/ClkInt < 10
                        warning('Adf24Tx2Rx4: Set TRampDo to 10 cycles')
                        Cfg.TRampDo     =   10*ClkInt;
                    end  							
					
					
                    switch(ClkSel)
                        case 1
                            obj.ADR7251Cfg(7)   =   hex2dec('00010000') + 100;
                            obj.ADR7251Cfg(8)   =   hex2dec('00020000') + 76;
                            obj.ADR7251Cfg(9)   =   hex2dec('00032833');   
                            obj.ADR7251Cfg(10)  =   hex2dec('01400000') + 3;  
							Cfg.Tp 				= 	ceil(Cfg.Tp/1e-6)*1e-6;
                    		Cfg.TRampUp 		= 	ceil(Cfg.TRampUp/1e-6)*1e-6;
                            
                        case 2
                            obj.ADR7251Cfg(7)   =   hex2dec('00010000') + 1000;
                            obj.ADR7251Cfg(8)   =   hex2dec('00020000') + 304;
                            obj.ADR7251Cfg(9)   =   hex2dec('00031003');    
                            obj.ADR7251Cfg(10)  =   hex2dec('01400000') + 3;  
							Cfg.Tp 				= 	ceil(Cfg.Tp/5e-6)*5e-6;
                    		Cfg.TRampUp 		= 	ceil(Cfg.TRampUp/5e-6)*5e-6	;						
                        case 3
                            obj.ADR7251Cfg(7)   =   hex2dec('00010000') + 1000;
                            obj.ADR7251Cfg(8)   =   hex2dec('00020000') + 304;
                            obj.ADR7251Cfg(9)   =   hex2dec('00031003');  
                            obj.ADR7251Cfg(10)  =   hex2dec('01400000') + 2;  
							Cfg.Tp 				= 	ceil(Cfg.Tp/5e-6)*5e-6;
                    		Cfg.TRampUp 		= 	ceil(Cfg.TRampUp/5e-6)*5e-6;								
                        otherwise
                    end
                    ...
                    obj.Rf_TRampUp              =   Cfg.TRampUp;					
                    obj.Rf_TRampDo              =   Cfg.Tp - Cfg.TRampUp; 
                    Cfg.TRampDo                 =   Cfg.Tp - Cfg.TRampUp; 					
                   
                    disp(['Sampling frequency: ', num2str(fsSel(ClkSel)/1e6), ' MSPS'])
                    % configure sampling clock to the timer unit
                    obj.ADIFMCWCfg(9)       =   ClkSel;
                    

                    
  
                    % Configure ticks which are ignored in sampling
                    Ticks               =   floor((Cfg.TRampUp + Cfg.TRampDo)/ClkInt);   
                    obj.ADIFMCWCfg(8)   =   Ticks   - 256 - 1 - 7; 
                    
                    obj.Dsp_SetAdiDefaultConf();

                    if isfield(Cfg, 'MimoEna')
                        if Cfg.MimoEna > 0
                            obj.Dsp_SetMimo(1);
                        end 
                    end
